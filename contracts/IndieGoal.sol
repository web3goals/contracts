    ...
    PausableUpgradeable
{
    ...
    function _closeAsFailedAfterDeadline(uint256 tokenId) internal {
        ...
        // If there are no motivators with points, then send stake to treasury
        if (totalMotivatorPoints == 0) {
            (bool sentToKepper, ) = _treasuryAddress.call{
                value: _params[tokenId].authorStake
            }("");
            if (!sentToKepper) revert Errors.SendingStakeToTreasuryFailed();
        }
        // If motivators with points exist, then send part of stake to them and part to treasury
        else {
            // Send stake to treasury
            uint stakeForTreasury = (_params[tokenId].authorStake *
                _usageFeePercent) / 100;
            (bool sentToKepper, ) = _treasuryAddress.call{
                value: stakeForTreasury
            }("");
            if (!sentToKepper) revert Errors.SendingStakeToTreasuryFailed();
